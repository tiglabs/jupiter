...
cql_rule_create(uint32_t ip, uint32_t qps) {
    struct cql_rule *r;
    uint32_t size;
    uint32_t i;

    size = sizeof(struct cql_rule) + qps * sizeof(struct cql_entry);
    r = rte_zmalloc(NULL, size, RTE_CACHE_LINE_SIZE);
    if (!r) {
        return NULL;
    }
    r->ip = ip;
...
struct lb_cql *
lb_cql_create(const char *name, uint32_t size) {
    struct lb_cql *cql;
    struct rte_hash_parameters params = {0};

    cql = rte_zmalloc(NULL, sizeof(struct lb_cql), 0);
    if (!cql) {
        return NULL;
    }
    cql->size = size;
